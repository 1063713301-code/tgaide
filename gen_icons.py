...
ok = skip = err = 0
for t in tools:
    # Skip if already has a real icon
    if t.get('icon_url') and t['icon_url'].startswith('http') and 'supabase' in t['icon_url']:
        skip += 1
        continue

    official_url = t.get('official_url', '').strip()
    if not official_url:
        skip += 1
        continue
    ...
    try:
        ...
        domain = urlparse(official_url).netloc or urlparse(official_url).path
...
print(f'Done: {ok} updated, {skip} skipped, {err} errors')
