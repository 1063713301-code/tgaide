...
# Fetch tools with empty keywords
req = urllib.request.Request(
    f'{URL}/rest/v1/tools?select=id,name,name_en,category&keywords=eq.&limit=500',
    headers={'apikey': KEY, 'Authorization': f'Bearer {KEY}'}
)
with urllib.request.urlopen(req) as r:
    tools = json.load(r)

print(f'Tools with empty keywords: {len(tools)}', flush=True)
