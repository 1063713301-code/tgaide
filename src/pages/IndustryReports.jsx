...
export default function IndustryReports() {
  ...
  useEffect(() => { document.title = '行业报告 - TG AI工具库' }, [])
  ...
  const load = useCallback(async (cat, pg) => {
    ...
    try {
      ...
      const [data, count] = await Promise.all([
        fetchArticles('report', { category: cat || null, limit: PAGE_SIZE, offset }),
        fetchArticleCount('report', cat || null),
      ])
      setArticles(data)
      setTotal(count)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-8">
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">📊 {t('reports_title')}</h1>
          <p className="text-gray-500 text-sm">{t('reports_page_sub')} · {t('articles_count')} {total}</p>
        </div>
