...
import { adminCreate, adminUpdate, adminFetchById } from '../../lib/supabase'
...
export default function AdminArticleEditor({ type, mode }) {
  ...
  const title = `${mode === 'new' ? '新建' : '编辑'}${isReport ? '行业报告' : '每日简报'}`
  const listPath = isReport ? '/admin/reports' : '/admin/briefs'
  ...
  const handleSave = async (statusOverride) => {
    ...
    try {
      if (mode === 'new') {
        ...
        await adminUpdate(type, id, payload)
      }
