...
export default function ReportList({ reportType }) {
  ...
  const load = useCallback(async (pg) => {
    ...
    try {
      ...
      const [data, count] = await Promise.all([
        fetchArticles('report', { reportType, limit: PAGE_SIZE, offset }),
        fetchReportCountByType(reportType),
      ])
      setArticles(data)
      setTotal(count)
  ...
  useEffect(() => { setPage(1) }, [reportType])
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-[1200px] mx-auto w-full px-4 sm:px-6 py-8">
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          ...
          <span>/</span>
          <Link to="/industry-reports" className="hover:text-blue-700">{t('reports_title')}</Link>
          <span>/</span>
          <span className="text-gray-600">{t(titleKey)}</span>
        </nav>
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">{t(titleKey)}</h1>
          <p className="text-gray-500 text-sm">{t(subKey)} · 共 {total} {t('articles_count')}</p>
        </div>
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {articles.map((item) => <ArticleCard key={item.id} article={item} type="report" />)}
          </div>
        )}
        {totalPages > 1 && (
